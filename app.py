...
def get_audio_from_video(video_path, audio_path):
    video = mpVideoFileClip(video_path)
    video.audio.write_audiofile(audio_path)

def get_text_from_audio(audio_path):
    whisper = pipeline("automatic-speech-recognition", model="openai/whisper-large-v3", torch_dtype=torch.float16, device="cuda:0")
...
def upload():
    ...
    return render_template('app_lang_select.html', video_path=video_path)
...
def translate():
    ...
    return render_template('app_result.html', original_video=video_path, translated_video=output_video_path)
